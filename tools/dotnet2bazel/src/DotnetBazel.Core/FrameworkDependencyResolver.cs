    ...
    internal class FrameworkDependencyResolver
    {
          ...
          IReadOnlyCollection<NugetRepositoryEntry> localPackages, string targetFramework)
        {
            ...
            foreach (var targetPackage in localTargetPackages)
            {
                ...
                entries.Add(
                  BuildEntry(targetPackage, targetFramework, frameworkList, upgrades, packageOverrides.Select(pair => $"{pair.Key}|{pair.Value}"))
                );
            }
        ...
        private NugetRepositoryEntry BuildEntry(
          ...
          IEnumerable<string> targetingPackOverrides)
        {
            ...
            entry.TargetingPackOverrides.AddRange(targetingPackOverrides);
            return entry;
        }
