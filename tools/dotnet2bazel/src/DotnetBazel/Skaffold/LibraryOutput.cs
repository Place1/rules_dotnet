...
{
    public static async Task WriteAsync(string name, string framework, string nugetRepo, IEnumerable<string> dependencies, TextWriter writer)
    {
        ...
        await writer.WriteLineAsync($"        \"@{nugetRepo}//microsoft.netcore.app.ref\",");
        await writer.WriteLineAsync($"        \"@{nugetRepo}//microsoft.aspnetcore.app.ref\",");
        await writer.WriteLineAsync("    ],");
