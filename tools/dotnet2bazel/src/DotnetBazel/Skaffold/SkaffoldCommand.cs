...
public sealed class SkaffoldCommandSettings : CommandSettings
{
    ...
    [CommandOption("--nuget-repo")]
    [Description("The name of the bazel nuget_repo to reference dependencies from.")]
    public string NugetRepositoryName { get; set; } = "nuget";
}
        ...
        await Task.WhenAll(settings.ProjectFiles
            .Select(async (projectFile) =>
            {
                ...
                var packageReferences = project.Items
                    ...
                    .Select(p => $"@{settings.NugetRepositoryName}//{p.Name.ToLower()}");
                ...
                {
                    await BinaryOutput.WriteAsync(name, settings.TargetFramework, settings.NugetRepositoryName, dependencies, output);
                }
                else
                {
                    await LibraryOutput.WriteAsync(name, settings.TargetFramework, settings.NugetRepositoryName, dependencies, output);
                }
